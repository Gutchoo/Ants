...
class Ant(Insect):
    ...
    def __init__(self, health=1, doubled_by_queen = False):
        """Create an Insect with a HEALTH quantity."""
        self.doubled_by_queen = doubled_by_queen
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while next_place.is_hive == False:
            if next_place.bees != [] and self.min_range <= counter <= self.max_range:
                counter =+ 1
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if cls.food_cost <= gamestate.food and gamestate.queen_created == False:
    ...
    def action(self, gamestate):
        ...
        while new_place != None:
            if new_place.ant != None and new_place.ant.doubled_by_queen == False:
                new_place.ant.damage = new_place.ant.damage*2
                new_place.ant.doubled_by_queen = True
            if new_place.ant != None and new_place.ant.is_container and new_place.ant.ant_contained != None and new_place.ant.ant_contained.doubled_by_queen == False:
                new_place.ant.ant_contained.damage *= 2
                new_place.ant.ant_contained.doubled_by_queen = True
...
class Bee(Insect):
    ...
    def __init__(self, health = 1):
        super().__init__(health)
        self.turns_to_be_slowed = 0
        self.has_been_scared = False
        self.length_of_scares = 0
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_be_slowed > 0:
            self.turns_to_be_slowed -= 1
        ...
        if self.length_of_scares > 0:
            if gamestate.time % 2 == 0 and self.turns_to_be_slowed == 0: 
                self.length_of_scares -= 1
                if self.blocked():
                    self.sting(self.place.ant)
                elif self.health > 0 and self.place.entrance.is_hive == False and self.place.entrance is not None:
                    self.move_to(self.place.entrance)
        
        if self.turns_to_be_slowed == 0 and self.length_of_scares == 0:
            if self.blocked():
                self.sting(self.place.ant)
            elif self.health > 0 and destination is not None:
                self.move_to(destination)
    ...
    def slow(self, length):
        ...
        # BEGIN Problem EC
        self.turns_to_be_slowed += length + 1
        # END Problem EC
    ...
    def scare(self, length):
        ...
        if self.has_been_scared == False:
            self.length_of_scares = length + 1
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2, queen_created = False):
        ...
        self.queen_created = queen_created
